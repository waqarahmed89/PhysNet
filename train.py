...
parser.add_argument(
    "--restart", type=str, default=None, help="restart training from a specific folder"
)
parser.add_argument(
    "--num_features", type=int, help="dimensionality of feature vectors"
)
...
parser.add_argument(
    "--grimme_s6", default=None, type=float, help="grimme s6 dispersion coefficient"
)
parser.add_argument(
    "--grimme_s8", default=None, type=float, help="grimme s8 dispersion coefficient"
)
parser.add_argument(
    "--grimme_a1", default=None, type=float, help="grimme a1 dispersion coefficient"
)
parser.add_argument(
    "--grimme_a2", default=None, type=float, help="grimme a2 dispersion coefficient"
)
...
parser.add_argument(
    "--max_norm", default=1000.0, type=float, help="max norm for gradient clipping"
)
parser.add_argument(
    "--ema_decay",
    default=0.999,
...
parser.add_argument(
    ...
    default=1.0,
    type=float,
    help="keep probability for dropout regularization of rbf layer",
)
parser.add_argument(
    "--l2lambda", type=float, help="lambda multiplier for l2 loss (regularization)"
)
parser.add_argument(
    "--nhlambda",
    type=float,
...
parser.add_argument(
    "--decay_rate",
    type=float,
    help="factor with which the learning rate gets multiplied by every decay_steps steps",
)
...
parser.add_argument(
    "--force_weight",
    default=52.91772105638412,
    type=float,
    help="this defines the force contribution to the loss function relative to the energy contribution (to take into account the different numerical range)",
)
parser.add_argument(
    "--charge_weight",
    default=14.399645351950548,
    type=float,
    help="this defines the charge contribution to the loss function relative to the energy contribution (to take into account the different numerical range)",
)
parser.add_argument(
    "--dipole_weight",
    default=27.211386024367243,
    type=float,
    help="this defines the dipole contribution to the loss function relative to the energy contribution (to take into account the different numerical range)",
)
parser.add_argument(
    "--summary_interval", type=int, help="write a summary every N steps"
)
parser.add_argument(
    "--validation_interval",
    type=int,
...
if args.restart is None:
    directory = (
        ...
        + str(args.num_blocks)
        + "a"
        + str(args.num_residual_atomic)
        + "i"
        + str(args.num_residual_interaction)
        + "o"
        + str(args.num_residual_output)
        + "cut"
        + str(args.cutoff)
        + "e"
        + str(args.use_electrostatic)
        + "d"
        + str(args.use_dispersion)
        + "l2"
        + str(args.l2lambda)
        + "nh"
        + str(args.nhlambda)
...
train_queue = DataQueue(
    data_provider.next_batch, capacity=1000, scope="train_data_queue"
)
valid_queue = DataQueue(
    data_provider.next_valid_batch,
    capacity=args.num_valid // args.valid_batch_size,
    scope="valid_data_queue",
)
...
energy_t, forces_t = nn.energy_and_forces_from_atomic_properties(
    Ea_t, Qa_t, Dij_t, Z_t, R_t, idx_i_t, idx_j_t, Qref_t, batch_seg_t
)
energy_v, forces_v = nn.energy_and_forces_from_atomic_properties(
...
with tf.name_scope("loss"):
    ...
    if data.Ea is not None:
        ...
        ealoss_t, eamse_t, eamae_t = (
            tf.constant(0.0),
            tf.constant(0.0),
            tf.constant(0.0),
        )
        ealoss_v, eamse_v, eamae_v = (
            tf.constant(0.0),
            tf.constant(0.0),
            tf.constant(0.0),
        )
    ...
    if data.Qa is not None:
        ...
        qaloss_t, qamse_t, qamae_t = (
            tf.constant(0.0),
            tf.constant(0.0),
            tf.constant(0.0),
        )
        qaloss_v, qamse_v, qamae_v = (
            tf.constant(0.0),
            tf.constant(0.0),
            tf.constant(0.0),
        )
    ...
    loss_t = (
        eloss_train
        + args.force_weight * floss_train
        + args.charge_weight * qloss_train
        + args.dipole_weight * dloss_train
        + args.nhlambda * nhloss_t
        + args.l2lambda * l2loss
    )
    loss_v = (
        eloss_valid
        + args.force_weight * floss_valid
        + args.charge_weight * qloss_valid
        + args.dipole_weight * dloss_valid
...
# create trainer
trainer = Trainer(
    args.learning_rate, args.decay_steps, args.decay_rate, scope="trainer"
)
...
with tf.Session() as sess:
    ...
    while not coord.should_stop():
        ...
        step += 1
        _, tmploss, emse, emae, fmse, fmae, qmse, qmae, dmse, dmae = sess.run(
            [
                train_op,
                loss_t,
                emse_t,
            ...
            options=run_options,
            feed_dict={nn.keep_prob: args.keep_prob},
        ...
        if step % args.validation_interval == 0:
            ...
            for i in range(args.num_valid // args.valid_batch_size):
                tmploss, emse, emae, fmse, fmae, qmse, qmae, dmse, dmae = sess.run(
                    [
                        loss_v,
                        emse_v,
                        emae_v,
            ...
            if data.E is not None:
                results["energy_mae_valid"] = emae_avg_v  # mean absolute error
                results["energy_rmse_valid"] = np.sqrt(  # root-mean-square error
                    emse_avg_v
                )
            if data.F is not None:
        ...
        if (step % args.summary_interval == 0) and (step > 0):
            ...
            if args.record_run_metadata > 0:
                summary_writer.add_run_metadata(
                    run_metadata, "step %d" % step, global_step=step
                )
            if data.E is not None:
                print(
                    str(step) + "/" + str(args.max_steps),
                    "loss:",
                    results["loss_train"],
                    "best:",
                    best_loss,
                    "emae:",
                    results["energy_mae_train"],
                    "best:",
                    best_emae,
                )
